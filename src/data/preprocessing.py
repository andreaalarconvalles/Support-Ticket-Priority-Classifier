...
def run_pipeline():
    ...
    def to_padded(texts):
        ...
        # Using pre-padding for LSTMs!
        return pad_sequences(sequences, maxlen=MAX_SEQ_LEN, padding="pre", truncating="post")

    X_train_text = to_padded(train_df["Ticket Description"])
    X_val_text = to_padded(val_df["Ticket Description"])
    X_test_text = to_padded(test_df["Ticket Description"])

    # Tabular Feature Encoding
    TABULAR_FEATURES = ['Ticket Status', 'Ticket Channel', 'Customer Gender', 'Ticket Subject', 'Product Purchased']
    encoder = OneHotEncoder(sparse_output=False, handle_unknown='ignore')
    
    # Fit only on train to prevent data leakage
    encoder.fit(train_df[TABULAR_FEATURES])
    
    X_train_tab = encoder.transform(train_df[TABULAR_FEATURES])
    X_val_tab = encoder.transform(val_df[TABULAR_FEATURES])
    X_test_tab = encoder.transform(test_df[TABULAR_FEATURES])
    ...
    np.save(PROCESSED_DIR / "X_train_text.npy", X_train_text)
    np.save(PROCESSED_DIR / "X_val_text.npy", X_val_text)
    np.save(PROCESSED_DIR / "X_test_text.npy", X_test_text)
    
    np.save(PROCESSED_DIR / "X_train_tabular.npy", X_train_tab)
    np.save(PROCESSED_DIR / "X_val_tabular.npy", X_val_tab)
    np.save(PROCESSED_DIR / "X_test_tabular.npy", X_test_tab)
    ...
    with open(PROCESSED_DIR / "encoder.pkl", "wb") as f:
    ...
    notes = (
        ...
        f"TABULAR_FEATURES_DIM = {X_train_tab.shape[1]}\n"
        ...
        "unseen words map to <OOV> (index 1). Padding changed to PRE.\n"
        "Tabular features OneHotEncoded and saved.\n"
    )
